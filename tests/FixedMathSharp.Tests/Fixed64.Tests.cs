...
public class Fixed64Tests
{
    ...
    {
        var value = Fixed64.CreateFromDouble(2.5);
        value = value.Offset(2);

        Assert.Equal(Fixed64.CreateFromDouble(4.5), value);
        Assert.Equal(value.rawValue.ToString(), value.RawToString());
    }
    ...
    public void ExplicitConversionsAndRawConverters_WorkCorrectly()
    {
        ...
        Assert.Equal(5L, (long)Fixed64.CreateFromDouble(5.75));

        Fixed64 fromInt = (Fixed64)6;
        Assert.Equal(new Fixed64(6), fromInt);
        Assert.Equal(6, (int)Fixed64.CreateFromDouble(6.75));

        Fixed64 fromFloat = (Fixed64)5.5f;
        Assert.Equal(Fixed64.CreateFromDouble(5.5), fromFloat);
        Assert.Equal(5.5f, (float)fromFloat);

        Fixed64 fromDouble = (Fixed64)7.25;
        Assert.Equal(Fixed64.CreateFromDouble(7.25), fromDouble);
        Assert.Equal(7.25, (double)fromDouble);

        Fixed64 fromDecimal = (Fixed64)8.5m;
        Assert.Equal(Fixed64.CreateFromDouble(8.5), fromDecimal);
        Assert.True(Math.Abs((decimal)fromDecimal - 8.5m) < 0.000001m);

        Assert.Equal(1.0, Fixed64.ToDouble(Fixed64.One.rawValue));
        Assert.Equal(1.0f, Fixed64.ToFloat(Fixed64.One.rawValue));
        Assert.True(Math.Abs(Fixed64.ToDecimal(Fixed64.One.rawValue) - 1.0m) < 0.000001m);
    }

    [Fact]
    public void MixedOperators_WorkCorrectly()
    {
        var value = Fixed64.CreateFromDouble(1.5);

        Assert.Equal(Fixed64.CreateFromDouble(3.5), value + 2);
        Assert.Equal(Fixed64.CreateFromDouble(3.5), 2 + value);

        Assert.Equal(Fixed64.CreateFromDouble(0.5), value - 1);
        Assert.Equal(Fixed64.CreateFromDouble(0.5), 2 - value);

        Assert.Equal(Fixed64.CreateFromDouble(3), value * 2);
        Assert.Equal(Fixed64.CreateFromDouble(3), 2 * value);
        Assert.Equal(Fixed64.CreateFromDouble(0.75), value / 2);
        Assert.Equal(Fixed64.CreateFromDouble(1.3333333333), 2 / value);
    }
    ...
    {
        var rawValue = Fixed64.One.rawValue.ToString();
        var negativeRawValue = (-Fixed64.One.rawValue).ToString();
    ...
    {
        var value = Fixed64.CreateFromDouble(1.2345);

        Assert.Equal("1.23", value.ToString("0.00"));
        Assert.Equal(0, value.CompareTo(Fixed64.CreateFromDouble(1.2345)));
        Assert.True(value.CompareTo(Fixed64.CreateFromDouble(1.2)) > 0);
        Assert.True(value.CompareTo(Fixed64.CreateFromDouble(1.3)) < 0);
    ...
    {
        var value = Fixed64.CreateFromDouble(-1.75);
        var positive = Fixed64.CreateFromDouble(1.75);
        ...
        Assert.True(Fixed64.CreateFromDouble(0.5).AbsLessThan(Fixed64.One));
    ...
    public void Fixed64Extensions_EpsilonAndFuzzyHelpers_WorkCorrectly()
    {
        ...
        Assert.True(new Fixed64(100).FuzzyComponentEqual(new Fixed64(101), Fixed64.CreateFromDouble(0.02)));
        Assert.False(new Fixed64(100).FuzzyComponentEqual(new Fixed64(103), Fixed64.CreateFromDouble(0.02)));
    }
